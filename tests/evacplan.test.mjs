...
test('evacuationPolicies: 浸水深と継続時間による方針の選択', () => {
  // 3m以上 → 立退き
  assert.deepEqual(evacuationPolicies({ floodIdx: 2, keizoku: false, landslide: LS_NONE }), [
    'policy.floodLeave',
  ]);
  // 0.5〜3m 単独 → 垂直避難も可
  assert.deepEqual(evacuationPolicies({ floodIdx: 1, keizoku: false, landslide: LS_NONE }), [
    'policy.floodVertical',
  ]);
  // 0.5〜3m + 長期浸水 → 立退き (在宅避難は危険)
  assert.deepEqual(evacuationPolicies({ floodIdx: 1, keizoku: true, landslide: LS_NONE }), [
    'policy.floodLeave',
    'policy.keizoku',
  ]);
  // 0.5m未満
  assert.deepEqual(evacuationPolicies({ floodIdx: 0, keizoku: false, landslide: LS_NONE }), [
    'policy.floodShallow',
  ]);
  // リスクなし
  assert.deepEqual(evacuationPolicies({ floodIdx: -1, keizoku: false, landslide: LS_NONE }), [
...
test('nearestShelters: 近い順にn件、フィルタと空フォールバック', () => {
  const shelters = [
    { lon: 135.7, lat: 34.6, name: 'A', disasters: ['洪水'] },
    { lon: 135.71, lat: 34.6, name: 'B', disasters: ['土砂'] },
    { lon: 135.695, lat: 34.6, name: 'C', disasters: [] }, // 未指定は常に候補
    { lon: 135.72, lat: 34.6, name: 'D', disasters: ['洪水', '土砂'] },
  ];
  const near = nearestShelters(shelters, 135.694, 34.6, '洪水', 2);
  assert.deepEqual(
    near.map((x) => x.shelter.name),
    ['C', 'A'],
  );
  assert.ok(near[0].dist < near[1].dist);
  // 該当フィルタなし → 全件から
  const fallback = nearestShelters(
    [{ lon: 135.7, lat: 34.6, name: 'X', disasters: ['土砂'] }],
    135.694,
    34.6,
    '洪水',
    2,
