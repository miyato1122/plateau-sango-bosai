...
import {
  edgeCost, buildGraph, nearestNode, findRoute, FLOOD_PENALTY,
...
const LADDER = {
  version: 1,
  nodes: [
    [135.690, 34.600], [135.693, 34.600], [135.696, 34.600],
    [135.690, 34.599], [135.693, 34.599], [135.696, 34.599],
  ],
  edges: [
    [0, 1, 275, -1, 0], [1, 2, 275, -1, 0],   // 北: 安全
    [3, 4, 275, 1, 0], [4, 5, 275, 1, 0],     // 南: 浸水クラス1
    [0, 3, 111, -1, 0], [2, 5, 111, -1, 0],   // 連絡路
  ],
};
...
test('findRoute: 浸水した近道より安全な迂回路を選ぶ', () => {
  ...
  const route = findRoute(LADDER, [135.690, 34.599], [135.696, 34.599]);
  assert.ok(route);
  assert.equal(route.riskyM, 0, '浸水区間を通らない');
  assert.ok(route.lengthM > 550, '遠回りを選んでいる');
  assert.deepEqual(route.coords[0], [135.690, 34.599]);
...
test('findRoute: 回避不能なら浸水区間を通り、危険距離を報告する', () => {
  const onlyFlooded = {
    ...LADDER,
    edges: [[3, 4, 275, 1, 0], [4, 5, 275, 1, 0]],
    nodes: LADDER.nodes,
  };
  const route = findRoute(onlyFlooded, [135.690, 34.599], [135.696, 34.599]);
  assert.ok(route);
...
test('findRoute: 到達不能・道路網外は null', () => {
  const split = {
    ...LADDER,
    edges: [[0, 1, 275, -1, 0], [4, 5, 275, 1, 0]], // 分断されたグラフ
  };
  assert.equal(findRoute(split, [135.690, 34.600], [135.696, 34.599]), null);
...
test('waysToGraph: OSM way→ノード・エッジ (重複way排除・bbox外除外)', () => {
  const osm = {
    elements: [
      { type: 'node', id: 10, lon: 135.69, lat: 34.60 },
      { type: 'node', id: 11, lon: 135.691, lat: 34.60 },
      { type: 'node', id: 12, lon: 135.692, lat: 34.60 },
